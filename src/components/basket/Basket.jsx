...
const Basket = () => {
  ...
  useEffect(() => {
    if (didMount && firebase.auth.currentUser && basket.length !== 0) {
      firebase.saveBasketItems(basket, firebase.auth.currentUser.uid)
        .then(() => console.log('Item saved to basket'))
  ...
  const handleApplyPromo = () => {
    if (promoCode.toLowerCase() === 'tools') {
      setIsApplied(true);
      alert('Promo code applied! You got 5% discount.');
    } else {
      setIsApplied(false);
  ...
  const finalTotal = isApplied ? subtotal * 0.95 : subtotal;
        ...
        <div className="basket-checkout">
          ...
          <div className="basket-total">
            <p className="basket-total-title">
              {isApplied ? 'Total (5% Discount Applied):' : 'Subtotal Amount:'}
            </p>
